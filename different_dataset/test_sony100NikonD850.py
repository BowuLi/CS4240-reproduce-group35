...
metric_average_file = 'metric_average_sony100NikonD850.txt'
# Intermediate details for the test images, such as estimated amplification will be saved here.
test_amplification_file = 'test_amplification_sony100NikonD850.txt'


# These are folders
save_images = 'images_sony100NikonD850'  # Restored images will be saved here.
# Other details such as loss value and learning rate will be saved in this file.
save_csv_files = 'csv_files_sony100NikonD850'
...
test_files = []
for i in range(10):
    test_files += glob.glob('NikonD850/scene-'+str(i+1)+'/IMG_0005.nef')

print(test_files)

gt_files = []
for i in range(10):
    gt_files += glob.glob('NikonD850/scene-'+str(i+1)+'/IMG_0001.nef')
