...
p = '/content/dataset/Sony/'

# train_files = glob.glob(p+'short/0000*_00_0.1s.ARW')
# train_files +=glob.glob(p+'short/0001*_00_0.1s.ARW')
# train_files +=glob.glob(p+'short/0002*_00_0.1s.ARW')
# train_files = glob.glob(p+'short/0*_00_0.1s.ARW')
# train_files +=glob.glob(p+'short/2*_00_0.1s.ARW')
# If you have less CPU RAM you would like to use fewer images for training.
# if dry_run:
#     train_files = train_files[:2]
#     opt['iterations'] = dry_run_iterations

# l = int(len(train_files)*0.2)
# l_prime = len(train_files) - l
# train_files,_=random_split(train_files, [l, l_prime], generator=torch.Generator().manual_seed(42))
  
# gt_files = []
# for x in train_files:
#     gt_files += glob.glob(p+'long/*'+x[-17:-12]+'*.ARW')
# print(len(gt_files))
...
# test_files = glob.glob(p+'short/10*_00_0.1s.ARW') 
# if dry_run:
#     test_files = test_files[:2]
    
# gt_files = []
# for x in test_files:
#     gt_files = gt_files+ glob.glob(p+'long/*'+x[-17:-12]+'*.ARW')
...
#   ll.append((img[0],img[1]))
# torch.save(ll, 'll.pt')
############
train_list = torch.load('train_list100.pt')
test_list = torch.load('test_list100.pt')
...
while iter_num<opt['iterations']:
    for _, img in enumerate(train_list):
        low = img[0].to(device)
        gt = img[1].to(device)
        # print(low.shape)
        # print(gt.shape)
        ...
        if iter_num in opt['atWhichSave']:
            ...
            if iter_num == opt['atWhichSave'][0]:
                ...
                mode = 'a'
            run_test(model, test_list, iter_num, save_images, save_csv_files, metric_average_file, mode, training=True)
